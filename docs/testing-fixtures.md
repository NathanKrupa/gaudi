...
## Activation Gates and Fixture Pitfalls

A rule's `check()` is only called when its **activation gates** pass — the
library-import gate (`requires_library`) and the philosophy-scope gate
(`philosophy_scope`). A fixture that fails to satisfy these gates is either
vacuous (proves nothing) or accidentally triggered (proves the wrong thing).
Three failure modes have been observed; fixture authors should watch for all
of them.

### 1. Vacuous Pass — rule never ran

A pass fixture for a library-gated rule must import that library. If it
doesn't, the rule's `check()` is never called, the runner sees zero findings,
and the fixture passes — but it proved nothing.

**Example:** A pass fixture for SA-SCALE-001 (SQLAlchemy) defines a local
`relationship()` function without importing `sqlalchemy`. The rule never
activates.

**Fix:** The corpus runner enforces a vacuous-pass assertion
(`test_pass_fixture_not_vacuous`). If a pass fixture doesn't satisfy the
library gate, the test fails with a clear message. Fixtures that
*intentionally* test the inactive gate — "rule correctly ignores code when
the library isn't present" — must signal this with a name containing
`no_<library>`, `non_<library>`, `not_<library>`, or `no_import`.

### 2. Accidental Activation — stripped name matches a path gate

The runner copies single-file fixtures into a temp project with the
`fail_`/`pass_` prefix stripped: `pass_non_settings_module.py` becomes
`non_settings_module.py`. Rules that key on filenames (e.g., DJ-SEC-001
checking for `settings` in the path) see the **stripped** name. If the
stripped name accidentally contains the activation substring, the rule fires
on a pass fixture that was never meant to trigger it.

**Example:** `pass_non_settings_module.py` → `non_settings_module.py` still
contains `"settings"`, so DJ-SEC-001 activated and the pass fixture failed.

**Fix:** Choose fixture names whose stripped form does not accidentally match
path-based activation patterns. The corpus runner logs the stripped name at
`INFO` level so authors can audit what the rule sees (run with
`--log-cli-level=INFO`).

### 3. Receiver-Blind AST Match — wrong object, right method name

A rule that matches on method name alone (e.g., `.extractall()`) without
verifying the receiver type will fire on any object with that method. A
coarse short-circuit like `if "tarfile" not in source` is defeated by
comments, docstrings, and variable names.

**Example:** PY314-006 flagged `zipfile.ZipFile.extractall()` in a file
whose docstring mentioned `"tarfile"`.

**Fix:** Track receiver bindings through `ast.Assign` and `ast.With` nodes,
then match method calls only when the receiver is in the tracked set. The
boto3 client-name tracking pattern in
`src/gaudi/packs/python/rules/boto3.py` is the canonical example.

### Debugging Activation

The corpus runner logs the activation triple for every fixture at `INFO`
level:

```
activation: fixture=pass_clean.py  stripped=clean.py  ran_rules=[PY-001, PY-002, ...]
```

To see this output, run:

```bash
pytest tests/test_fixture_corpus.py --log-cli-level=INFO -k <RULE-ID>
```

If the rule under test does not appear in `ran_rules`, the fixture isn't
satisfying that rule's activation gate.
